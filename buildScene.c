...
void buildWalls(void) {
  ...
  // the image.
  //
  // Each 'wall' is really a ray, the origin p is one of the endpoints of the
  // wall, and the direction is scaled so that p + lambda*d gives you the second
  // endpoint for lambda=1. The 4 walls make up a closed loop and the directions
  // are as shown above. This will be imporant for you when you need to
  // determine the normal vector for each wall - you must make sure the vector
  // points toward the *inside* of the box!
  //
  // You must also make sure that all objects you define for the scene fall
  // within the window defined by W_TOP, W_LEFT, W_RIGHT, and W_BOTTOM.
  ...
  walls[0].w.p = p;
  walls[0].w.d = d;
  walls[0].material_type =
      1; // Change to 0 to make the walls mirror-reflecting!
...
void buildScene(void) {
  ...
  l.d = d;
  lightsource.l = l;
  lightsource.light_type =
      0; // Change this to set the light source type to point or laser
  lightsource.R =
      1.0; // Lightsource colour in RGB space. All values must be in [0,1]
