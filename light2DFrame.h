#ifndef LIGHT2DFRAME_H
#define LIGHT2DFRAME_H
...
//#define FRAME_STEP 0.01

struct DynamicFrameArray frameArray; 

double FRAME_STEP = 0.0001;
...
struct ray2D {       // Rays in 2D
  ...
  int monochromatic; // Flag to indicate whether this ray is monochromatic (true
                     // if set to 1)
...
struct wall2D { // Walls in 2D
  struct ray2D
      w; // The actual wall is just a ray, the origin is one endpoint, and
         // origin + d gives the second endpoind (i.e. with lambda=1)
  int material_type; // Type of material, can only be 0 or 1 with the same
                     // meaning as for circles
};
...
struct circ2D
    objects[MAX_OBJECTS]; // Array to hold scene objects (up to MAX_OBJECTS)
...
void hue2RGB(double H, double *R, double *G,
             double *B);  // Returns colour for a given hue
void renderObjects(void); // Renders objects onto image
// This function renders a ray onto the image between the two given points, with
// the specified RGB colour
...
void setPixel(double x, double y, double R, double G,
              double B); // Set image pixel at (x,y) to the
                         // specified colour.
double dot(struct point2D *p,
           struct point2D *q);     // Dot product between vectors p and q
void normalize(struct point2D *d); // Normalize a vector to unit length
void addCirc(struct point2D *c, double r, int type,
             double n); // Inserts a circle into the objects array
void buildWalls(void);  // This function makes the walls of the box. Implemented
                        // in buildScene.c
